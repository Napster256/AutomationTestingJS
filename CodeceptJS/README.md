...
4. обновление типов IntelliSense CodeceptJS `I` object `steps.d.ts`
  ...
  **Helper** — это вспомогательный технический код, который помогает тестам работать, но сам по себе не является бизнес-сценарием.

  **Steps** — это бизнес-действия пользователя или сценарные шаги, которые описывают поведение системы понятным языком в [BDD](https://testengineer.ru/pom-bdd-ddt-in-playwright-automation/) стиле и патерне [ААА](https://testengineer.ru/aaa-pattern/).